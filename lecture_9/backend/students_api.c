...
static int get_students_by_group_handler(struct mg_connection *conn, void *userdata) {
    sqlite3 *db;
    char *zErrMsg = 0;
    int rc;
    char *sql;
    char group[256]; // Buffer to store the group param
    const char *query_string = mg_get_request_info(conn)->query_string; // Get the query string.

    if (query_string == NULL || mg_get_var(query_string, strlen(query_string), "group", group, sizeof(group)) <= 0) {
        mg_printf(conn, "HTTP/1.1 400 Bad Request\r\nContent-Type: application/json\r\nAccess-Control-Allow-Origin: *\r\n\r\n{\"error\":\"City parameter missing\"}");
        return 400;
    }
    ...
    sql = sqlite3_mprintf("SELECT * FROM Students WHERE grp = '%q';", group); // Use sqlite3_mprintf to avoid SQL injection.
    cJSON *json_array = cJSON_CreateArray();
...
int main() {
    ...
    mg_set_request_handler(ctx, "/api/v1/get_students_by_group", get_students_by_group_handler, NULL);
